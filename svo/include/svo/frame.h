...
public:
  ...
  void initFrame(const cv::Mat& img);  //初始化新的帧并创建图像金字塔
  ...
  //遍历fts_中的所有特征，对每个特这个进行checkKeyPoint，选出5个关键点，对应于key_pts_;
  //这5个点的feature是位于图像4个角和中心的点，且有对应的3D points。
  void setKeyPoints();

  /// Check if we can select five better key-points.
  // 中间点要尽可能离中心近，四个角的点要尽量离中心点远。key_pts_中的五个点的顺序一次是中间，右上，右下，左下，左上
  ...
  //返回该帧在世界坐标系下的位姿（位移）
  inline Vector3d pos() const { return T_f_w_.inverse().translation(); }//T_f_w_.inverse()？？？？？？？？？

  /// Frame jacobian for projection of 3D point in (f)rame coordinate to
  /// unit plane coordinates uv (focal length = 1).
  inline static void jacobian_xyz2uv(  //十四讲p164公式
