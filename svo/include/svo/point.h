...
public:
  ...
  list<Feature*>              obs_;                     //!< References to keyframes which observe the point.
               // ！！！！该Point能被多个关键帧观测到；obs_存储了能观测到该点的关键帧中对应的feature
  ...
  void addFrameRef(Feature* ftr);       //// 操作：obs_.push_front(ftr), n_obs_++;
  ...
  Feature* findFrameRef(Frame* frame);  ////遍历obs_，如果obs_中的feature对应的frame是该传入的帧，则返回对应的feature


  /// Get Frame with similar viewpoint.
  //pos为frame的空间位置。
  //遍历obs_, 用于检测与当前帧同时观测到某3D点的夹角最小的关键帧，并将夹角最小的关键帧对应的feature传给obs。（在Fearue alignment中需要）。 
  //当角度大于60度，认为无相近关键帧，返回false。
