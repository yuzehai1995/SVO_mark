...
struct Feature
{
  ...
  FeatureType type;     //!< Type can be corner or edgelet.  是角点或是边缘点
  Frame* frame;         //!< Pointer to frame in which the feature was detected.  能观测到该特征的帧（对应的指针）
  Vector2d px;          //!< Coordinates in pixels on pyramid level 0.
  Vector3d f;           //!！！！< Unit-bearing vector of the feature.   // f = [(u-ox)/fx , (v-oy)/fy , 1] ;  f * depth --> [x , y , z] :    f表示尺度？？
  int level;            //!< Image pyramid level where feature was extracted.
  Point* point;         //!< Pointer to 3D point which corresponds to the feature.  该特征对应的3D点
  Vector2d grad;        //!< Dominant gradient direction for edglets, normalized.梯度方向
