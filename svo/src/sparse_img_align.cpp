...
//返回的是 前一帧所有特征点块(feature patch)像素投影后在cur_frame中特征点块数量
size_t SparseImgAlign::run(FramePtr ref_frame, FramePtr cur_frame)
{
  ...
  jacobian_cache_.resize(Eigen::NoChange, ref_patch_cache_.rows*patch_area_);//列数为ref_patch_cache的大小。每一列为一个#像素#对应的jacobian。
  ...
  for(level_=max_level_; level_>=min_level_; --level_)  //迭代优化
  {
    mu_ = 0.1;
    ...
    if(verbose_)
      printf("\nPYRAMID LEVEL %i\n---------------\n", level_);
    optimize(T_cur_from_ref);  //optmize在point.h中
  }
  cur_frame_->T_f_w_ = T_cur_from_ref * ref_frame_->T_f_w_;



  return n_meas_/patch_area_; //n_meas_表示前一帧所有特征点块(feature patch)像素投影后在cur_frame中的像素个数。特征点数还要用n_meas_ / patch_area_获得
...
void SparseImgAlign::precomputeReferencePatches()
{
  ...
  const cv::Mat& ref_img = ref_frame_->img_pyr_.at(level_);	//图像指针
  const int stride = ref_img.cols;							//参考图像的像素列数
  ...
  for(auto it=ref_frame_->fts_.begin(), ite=ref_frame_->fts_.end();//对每个feature进行操作
      it!=ite; ++it, ++feature_counter, ++visiblity_it)
  {
    // check if reference features is within current image
    const float u_ref = (*it)->px[0]*scale;
    const float v_ref = (*it)->px[1]*scale;
    const int u_ref_i = floorf(u_ref);		//floorf（x）取不大于x的最大整数
    const int v_ref_i = floorf(v_ref);
    if((*it)->point == NULL || u_ref_i-border < 0 || v_ref_i-border < 0 || u_ref_i+border >= ref_img.cols || v_ref_i+border >= ref_img.rows)
      continue;//若满足括号内任意一个，说明参考帧的该feature不在此图像内，直接进入下一循环
    *visiblity_it = true; //否则参考帧该feature在当前图像内，标识符置为true，进行后续操作...
	...
	//上述f定义在feature.h中———— Vector3d  f;           //< Unit-bearing vector of the feature.    f = [(u-ox)/fx , (v-oy)/fy , 1] ;  f * depth --> [x , y , z] : 3d 坐标in this frame


    // evaluate projection jacobian
    Matrix<double,2,6> frame_jac;  //！！！！！！！！！！！！
    Frame::jacobian_xyz2uv(xyz_ref, frame_jac); //定义在frame.h中，是十四讲p164公式，从投影误差对位姿的雅各比

    // compute bilateral interpolation weights for reference image  双边插值权重计算
    const float subpix_u_ref = u_ref-u_ref_i;
    const float subpix_v_ref = v_ref-v_ref_i;
    const float w_ref_tl = (1.0-subpix_u_ref) * (1.0-subpix_v_ref);//左上点的权重
    const float w_ref_tr = subpix_u_ref * (1.0-subpix_v_ref);//右上点的权重
    const float w_ref_bl = (1.0-subpix_u_ref) * subpix_v_ref;//左下点的权重
    const float w_ref_br = subpix_u_ref * subpix_v_ref;//右下的权重
    size_t pixel_counter = 0;

	//ref_patch_cache是参考帧features对应的缓存区  ref_patch_cache_ = cv::Mat(ref_frame_->fts_.size(), patch_area_, CV_32F);
	//cache_ptr 是指向ref_patch_cache中当前feature的指针
    float* cache_ptr = reinterpret_cast<float*>(ref_patch_cache_.data) + patch_area_*feature_counter;

	//以下循环对当前feature所在的patch_area(4X4)进行遍历，分别计算每块的雅各比
    for(int y=0; y<patch_size_; ++y)
    {
		//（为什么要减去patch_halfsize_）？？？画图即可看出
      ...
      for(int x=0; x<patch_size_; ++x, ++ref_img_ptr, ++cache_ptr, ++pixel_counter)
      {
        ...
        *cache_ptr = w_ref_tl*ref_img_ptr[0] + w_ref_tr*ref_img_ptr[1] + w_ref_bl*ref_img_ptr[stride] + w_ref_br*ref_img_ptr[stride+1];
        ...
        // cache the jacobian  ？？？？？？？frame_jac是投影误差对位姿的雅各比
        jacobian_cache_.col(feature_counter*patch_area_ + pixel_counter) =  //（feature对应的）patch相应的pixel 的雅各比
            (dx*frame_jac.row(0) + dy*frame_jac.row(1))*(focal_length / (1<<level_));//对frame的雅各比进行变换
      }
    }
  }
    ...
    bool compute_weight_scale)
{
      ...
      ++it, ++feature_counter, ++visiblity_it)
  {
    ...
    const double depth = ((*it)->point->pos_ - ref_pos).norm();
    const Vector3d xyz_ref((*it)->f*depth);   //该特征在参考帧的（相机坐标系下）3D坐标
    const Vector3d xyz_cur(T_cur_from_ref * xyz_ref);//该特征在当前帧的（相机坐标系下）3D坐标
    const Vector2f uv_cur_pyr(cur_frame_->cam_->world2cam(xyz_cur).cast<float>() * scale);
    const float u_cur = uv_cur_pyr[0];//像素坐标
    ...
    // compute bilateral interpolation weights for the current image  //像素坐标落在周围四个点的权重
    ...
    for(int y=0; y<patch_size_; ++y)
    {
      ...
      for(int x=0; x<patch_size_; ++x, ++pixel_counter, ++cur_img_ptr, ++ref_patch_cache_ptr)
      {
        ...
        if(linearize_system)
        {
          ...
          const Vector6d J(jacobian_cache_.col(feature_counter*patch_area_ + pixel_counter));   //用参考帧的雅可比作为当前帧的雅可比矩阵！！
          H_.noalias() += J*J.transpose()*weight;   //海塞矩阵H p294计算过程     //H在哪里定义的？？？库？？
          Jres_.noalias() -= J*res*weight;          //g或者b
          if(display_)
