    ...
    bool no_simd)
{
  ...
  float __attribute__((__aligned__(16))) ref_patch_dy[patch_area_]; // 定义一个16字节对齐的变量
  ...
  // Compute pixel location in new image:
  float u = cur_px_estimate.x();   // 特征块中心像素点在新图像中的位置
  float v = cur_px_estimate.y();
...
//    float new_chi2 = 0.0;
    ...
    for(int y=0; y<patch_size_; ++y)
    {
      ...
      {
        float search_pixel = wTL*it[0] + wTR*it[1] + wBL*it[cur_step] + wBR*it[cur_step+1]; // 在cur中的像素值，双线性插值
        float res = search_pixel - *it_ref + mean_diff; // cur 和ref 中像素差值。加上mean_diff  ?        Jres[0] -= res*(*it_ref_dx);
